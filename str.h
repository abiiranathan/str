...
size_t str_len(const str* s) { return s ? s->length : 0; }

size_t str_capacity(const str* s) { return s ? s->capacity : 0; }

bool str_empty(const str* s) { return !s || s->length == 0; }

bool str_ensure_capacity(str** s, size_t capacity) {
    if (!s || !*s) return false;

    if ((*s)->capacity >= capacity) return true;
...
bool str_append(str** s, const char* append) {
    if (!s || !*s || !append) return false;
    size_t append_len = strlen(append);
    if (!str_ensure_capacity(s, (*s)->length + append_len + 1)) return false;
...
bool str_append_fmt(str** s, const char* format, ...) {
    ...
    if (!str_ensure_capacity(s, (*s)->length + size + 1)) return false;
...
bool str_append_char(str** s, char c) {
    if (!s || !*s) return false;
    if (!str_ensure_capacity(s, (*s)->length + 2)) return false;
...
bool str_prepend(str** s, const char* prepend) {
    if (!s || !*s || !prepend) return false;
    size_t prepend_len = strlen(prepend);
    if (!str_ensure_capacity(s, (*s)->length + prepend_len + 1)) return false;
...
bool str_insert(str** s, size_t index, const char* insert) {
    if (!s || !*s || !insert || index > (*s)->length) return false;
    size_t insert_len = strlen(insert);
    if (!str_ensure_capacity(s, (*s)->length + insert_len + 1)) return false;
...
bool str_remove(str** s, size_t index, size_t count) {
    if (!s || !*s || index >= (*s)->length) return false;
...
bool str_resize(str** s, size_t new_length) {
    if (!s || !*s) return false;
    if (!str_ensure_capacity(s, new_length + 1)) return false;
...
char str_at(const str* s, size_t index) { return (s && index < s->length) ? s->data[index] : '\0'; }

char* str_data(str* s) { return s ? s->data : NULL; }

const char* str_cstr(const str* s) { return s ? s->data : NULL; }
...
bool str_equals(const str* s1, const str* s2) { return str_compare(s1, s2) == 0; }
...
int str_rfind(const str* s, const char* substr) {
    if (!s || !substr || !*substr) return STR_NPOS;

    size_t substr_len = strlen(substr);
    if (substr_len > s->length) return STR_NPOS;
...
void str_trim(str* s) {
    ...
    while (end > start && isspace(s->data[end])) --end;
...
void str_rtrim(str* s) {
    ...
    while (end > 0 && isspace(s->data[end])) --end;
...
str** str_split(const str* s, const char* delim, size_t* count) {
    ...
    *count = 0;
    ...
    while ((end = strstr(start, delim))) {
        ...
        if (!result[*count]) {
            for (size_t i = 0; i < *count; ++i) str_free(result[i]);
            free(result);
            return NULL;
        }
